...
local apply_highlights = function(colorscheme)
  for _, hi in pairs(require("highlights").highlights[colorscheme]) do
    if hi.clear then
      vim_cmd(("highlight clear %s"):format(hi.name))
    end

    local guifg = (type(hi.guifg) == "string" and hi.guifg)
      or (type(hi.guifg) == "table" and get_hex(
        hi.guifg[1],
        hi.guifg[2]
      ))
      or nil

    local guibg = (type(hi.guibg) == "string" and hi.guibg)
      or (type(hi.guibg) == "table" and get_hex(
        hi.guibg[1],
        hi.guibg[2]
      ))
      or nil
    ...
    vim_cmd(
      hi.link and ("highlight link %s %s"):format(hi.name, hi.link)
...
_G.autocmd = function(autocmd)
  vim_cmd(
    ("autocmd %s %s %s"):format(autocmd.event, autocmd.pattern, autocmd.cmd)
  )
end
...
---@param filename  string
local macos_open_tex_pdf = function(filename) end
